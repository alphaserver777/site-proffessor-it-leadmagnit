...
Рабочая машина подключается к внутреннему адресу `192.168.50.111` через
Proxmox как промежуточный узел:

```bash
ssh -J Proxmox deploy@192.168.50.111
```

Этот маршрут уже записан в рабочем inventory, поэтому обычная команда
публикации использует его автоматически. При ручном запуске из другого
inventory передайте Ansible параметр подключения и явный идентификатор выпуска:
